...
serve(async (req) => {
  ...
  try {
    ...
    if (invoice.status === "paid") return jsonResponse({ status: "settlement", already_paid: true });
    ...
    if (!payment) return jsonResponse({ status: "pending", message: "Sesi pembayaran belum ditemukan" });
    if (payment.status === "settlement") return jsonResponse({ status: "settlement", already_paid: true });
    ...
    const payload = await readResponsePayload(response);
    if (!response.ok) {
      console.error("pivot-status rejected:", JSON.stringify(payload));
    ...
    if (status === "settlement") {
      ...
      if (!amountMatches || !currencyMatches) {
        console.error("pivot-status settlement mismatch", {
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .eq("id", payment.id);
    if (updateError) {
      console.error("pivot-status update failed:", updateError.message);
      return jsonResponse({ error: updateError.message }, 500);
    }

    return jsonResponse({ status });
  } catch (error) {
    console.error("pivot-status error:", error);
    return jsonResponse(
